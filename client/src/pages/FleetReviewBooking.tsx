...
const ROLE_OPTIONS: Array<{ value: string; label: string; decisionMaker: boolean }> = [
  { value: "owner", label: "Fleet owner / principal", decisionMaker: true },
  { value: "ops", label: "Operations manager", decisionMaker: true },
  { value: "maintenance", label: "Maintenance / fleet manager", decisionMaker: true },
  { value: "influencer", label: "I influence maintenance decisions", decisionMaker: true },
  { value: "not_involved", label: "Just researching / not a decision-maker", decisionMaker: false },
];
const FLEET_SIZE_OPTIONS: Array<{ value: string; label: string; qualified: boolean }> = [
...
const REVIEW_EXPECTATIONS: Array<{ icon: typeof Search; title: string; body: string }> = [
  {
    icon: Search,
...
const AGENDA: Array<[string, string]> = [
  ["0–3 min", "Context: fleet size, asset types, who owns maintenance, and your primary concern."],
  ["3–15 min", "Workflow diagnosis: how issues are reported, prioritized, approved, assigned, tracked, and confirmed — and what management can't currently see."],
  ["15–22 min", "Tailored demonstration of the workflows connected to the gaps we find."],
  ["22–25 min", "An honest recommendation: pilot-ready, improve first, or not currently a fit."],
];
...
function FounderCard() {
  return (
    <div className={cn(cardClass, "flex items-start gap-4 p-5")}>
      ...
      <div>
        <p className={cn(eyebrowClass, "!text-[#73777E]")}>Your review is led by</p>
        <p className="mt-1 text-base font-bold text-[#0A1A2E]">Dickson Kwesiga</p>
        <p className="text-sm text-[#73777E]">Founder, TruckFixr Fleet AI</p>
        <p className="mt-2 text-sm leading-6 text-[#38465F]">
          Hands-on experience managing commercial fleets and heavy-duty repair operations — you&apos;re booking a
          working session with the founder, not a sales rep.
        </p>
      </div>
    </div>
  );
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <h1 className={cn(displayClass, "mt-2 text-3xl sm:text-4xl")}>Book your Fleet Maintenance Review</h1>
        <p className="mt-4 text-base leading-7 text-[#38465F]">
          A focused working session to find where maintenance issues are getting lost, delayed, or turning into costly
          downtime in your operation — and to show how TruckFixr could close those gaps.
        </p>
        <p className="mt-3 text-sm text-[#73777E]">
          For commercial fleets with <span className="font-semibold text-[#0A1A2E]">5–50 vehicles</span>. No obligation.
        </p>
      </div>
          ...
          return (
            <div key={e.title} className={cn(cardClass, "flex gap-3 p-4")}>
              ...
              <div>
                ...
                <p className="mt-1 text-sm leading-6 text-[#38465F]">{e.body}</p>
              </div>
            </div>
          );
        })}
      ...
      <div className={cn(cardClass, "bg-[#F6F8FB] p-5")}>
        <p className={cn(eyebrowClass, "!text-[#73777E]")}>How the 25 minutes are spent</p>
        <dl className="mt-3 space-y-2.5">
          {AGENDA.map(([time, body]) => (
            <div key={time} className="flex gap-3">
              <dt className="w-16 shrink-0 font-['JetBrains_Mono'] text-xs font-bold text-[#D81F2A]">{time}</dt>
        ...
        <p className="mt-4 flex items-start gap-2 border-t border-[#E2E6EC] pt-3 text-xs leading-5 text-[#73777E]">
          <ClipboardCheck className="mt-0.5 h-4 w-4 shrink-0 text-[#1EA66C]" aria-hidden="true" />
          Come with one recent maintenance issue, recurring fault, inspection concern, or repair decision that was hard
          to manage. Invoices or inspection records are welcome but optional.
        </p>
      </div>
      ...
      <p className="text-xs leading-5 text-[#73777E]">
        If there&apos;s a strong operational fit, we may recommend a small, standardized paid pilot at the end. There is
...
function FieldLabel({ htmlFor, children, optional = false }: { htmlFor: string; children: ReactNode; optional?: boolean }) {
  return (
    <Label htmlFor={htmlFor} className="mb-1.5 flex items-center gap-1.5 text-sm font-semibold text-[#0A1A2E]">
      {children}
      {optional ? <span className="text-xs font-normal text-[#9AA3B0]">(optional)</span> : null}
    </Label>
  );
}
...
function QualificationForm({ onQualified, onNurture }: {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    trackEvent(qualifies ? "fleet_review_qualified" : "fleet_review_not_qualified", {
      role: form.role,
  ...
  return (
    <form onSubmit={handleSubmit} className={cn(cardClass, "space-y-4 p-5 sm:p-6")} noValidate>
      <div>
        <p className={eyebrowClass}>Step 1 of 2 · Tell us about your fleet</p>
        <p className="mt-1 text-sm leading-6 text-[#38465F]">
          A few questions so the review is worth your time. If it&apos;s a fit, the calendar appears next.
        </p>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        <div>
          ...
          <Input id="name" value={form.name} onChange={(e) => set("name", e.target.value)} className={fieldClass} autoComplete="name" />
        </div>
        <div>
          <FieldLabel htmlFor="company">Company</FieldLabel>
          <Input id="company" value={form.company} onChange={(e) => set("company", e.target.value)} className={fieldClass} autoComplete="organization" />
        </div>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <div>
          ...
          <select id="fleetSize" value={form.fleetSize} onChange={(e) => set("fleetSize", e.target.value)} className={selectClass}>
      ...
      <div>
        <FieldLabel htmlFor="handling" optional>How maintenance and repairs are handled</FieldLabel>
        <select id="handling" value={form.handling} onChange={(e) => set("handling", e.target.value)} className={selectClass}>
      ...
      <div>
        <FieldLabel htmlFor="tracking" optional>How issues and repairs are currently tracked</FieldLabel>
        <Textarea id="tracking" value={form.tracking} onChange={(e) => set("tracking", e.target.value)} className={fieldClass} rows={2} placeholder="Spreadsheets, paper, texts/calls, an existing tool…" />
      </div>

      <div>
        <FieldLabel htmlFor="challenge">Biggest maintenance challenge right now</FieldLabel>
        <Textarea id="challenge" value={form.challenge} onChange={(e) => set("challenge", e.target.value)} className={fieldClass} rows={3} placeholder="What keeps slipping, getting missed, or costing you unplanned downtime?" />
      </div>

      <div className="grid gap-4 sm:grid-cols-2">
        <div>
          <FieldLabel htmlFor="email">Best email</FieldLabel>
          <Input id="email" type="email" value={form.email} onChange={(e) => set("email", e.target.value)} className={fieldClass} autoComplete="email" inputMode="email" />
        </div>
        <div>
          <FieldLabel htmlFor="phone" optional>Phone</FieldLabel>
          <Input id="phone" type="tel" value={form.phone} onChange={(e) => set("phone", e.target.value)} className={fieldClass} autoComplete="tel" inputMode="tel" />
        </div>
      </div>

      <label htmlFor="pilotOpen" className="flex cursor-pointer items-start gap-3 rounded-md border border-[#C3C7CE] bg-[#F6F8FB] p-3">
        <Checkbox
          id="pilotOpen"
        ...
        <span className="text-sm leading-6 text-[#38465F]">
          If there is a clear operational fit, I am open to considering a small paid pilot.
        </span>
      </label>

      {error ? <p className="text-sm font-semibold text-[#D81F2A]">{error}</p> : null}

      <Button type="submit" className={cn("h-12 w-full justify-center font-bold", redBtn)}>
...
function BookStage({ form }: { form: FormState }) {
  const src = useMemo(() => {
    ...
    try {
      ...
      const sizeLabel = FLEET_SIZE_OPTIONS.find((s) => s.value === form.fleetSize)?.label ?? form.fleetSize;
      ...
      url.searchParams.set("a1", `${form.company} · ${sizeLabel} · ${roleLabel}`);
  ...
  return (
    <div className="space-y-4">
      <div className={cn(cardClass, "flex items-start gap-3 border-l-[3px] border-l-[#1EA66C] p-4")}>
        <span className="mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-[#1EA66C]">
          <Check className="h-4 w-4 text-white" aria-hidden="true" />
        </span>
        <div>
          <p className="text-sm font-bold text-[#0A1A2E]">You&apos;re a fit — pick a time below.</p>
          <p className="mt-1 text-sm leading-6 text-[#38465F]">
            Thanks, {form.name.split(" ")[0] || "there"}. Choose a slot that works and you&apos;ll get a calendar invite
            with a video link. Bring one real maintenance example.
          </p>
        </div>
      </div>
      ...
      {src ? (
        <div className={cn(cardClass, "overflow-hidden p-0")}>
          <iframe src={src} title="Book your Fleet Maintenance Review" loading="lazy" className="h-[720px] w-full border-0 bg-white" />
        </div>
      ) : (
        <div className={cn(cardClass, "flex flex-col items-center gap-3 p-8 text-center")}>
          <CalendarClock className="h-8 w-8 text-[#D81F2A]" aria-hidden="true" />
          <p className="text-sm font-bold text-[#0A1A2E]">Scheduling is being set up.</p>
          <p className="max-w-sm text-sm leading-6 text-[#38465F]">
            Reply to the confirmation email we&apos;ll send, or reach out and we&apos;ll lock in a time for your review.
          </p>
        </div>
      )}
    </div>
...
function NurtureStage({ form, onBack }: { form: FormState; onBack: () => void }) {
  const sizeMeta = FLEET_SIZE_OPTIONS.find((s) => s.value === form.fleetSize);
  const roleMeta = ROLE_OPTIONS.find((r) => r.value === form.role);
  const reason =
    !roleMeta?.decisionMaker
  ...
  return (
    <div className={cn(cardClass, "space-y-4 p-6")}>
      ...
      <div>
        <h2 className={cn(displayClass, "text-2xl")}>Thanks — let&apos;s stay in touch</h2>
        <p className="mt-2 text-sm leading-6 text-[#38465F]">
          {reason} We&apos;d rather be honest than book a call that isn&apos;t right for you.
        </p>
      </div>
      <div className="rounded-md border border-[#E2E6EC] bg-[#F6F8FB] p-4 text-sm leading-6 text-[#38465F]">
        In the meantime, our free maintenance planning guides and inspection checklists are open to everyone.
      </div>
      <div className="flex flex-col gap-3 sm:flex-row">
        <Link href="/resources">
          <Button variant="outline" className="h-11 w-full justify-center border-[#0A1A2E] font-bold text-[#0A1A2E] hover:bg-[#0A1A2E] hover:text-white sm:w-auto sm:px-6">
            Browse free resources
            <ArrowRight className="ml-2 h-4 w-4" />
          </Button>
        </Link>
        <Button variant="ghost" onClick={onBack} className="h-11 justify-center font-semibold text-[#73777E]">
...
export default function FleetReviewBooking() {
  ...
  return (
    <div className="min-h-screen bg-[#F6F8FB] font-['IBM_Plex_Sans'] text-[#0A1A2E]">
      ...
      <div className={cn(sectionShell, "grid gap-10 py-10 lg:grid-cols-[0.95fr_1.05fr] lg:py-16")}>
        <LeftColumn />
        <div>
          <div className="mb-4 flex flex-wrap items-center gap-x-5 gap-y-1 text-xs font-semibold text-[#73777E]">
            <span className="inline-flex items-center gap-1.5"><Clock className="h-4 w-4 text-[#2D7DE0]" aria-hidden="true" /> 25 minutes</span>
            <span className="inline-flex items-center gap-1.5"><ShieldCheck className="h-4 w-4 text-[#1EA66C]" aria-hidden="true" /> No obligation</span>
            <span className="inline-flex items-center gap-1.5"><Mail className="h-4 w-4 text-[#73777E]" aria-hidden="true" /> Founder-led</span>
          </div>
          {stage === "form" ? (
            <QualificationForm
              onQualified={(f) => { setForm(f); setStage("book"); }}
              onNurture={(f) => { setForm(f); setStage("nurture"); }}
            />
          ) : stage === "book" ? (
            <BookStage form={form} />
          ) : (
      ...
      <p className="mx-auto max-w-[1120px] px-4 pb-3 text-center text-xs leading-5 text-[#73777E] sm:px-6">
        Built and tested through real commercial fleet and heavy-duty maintenance workflows. TruckFixr provides decision
        support and does not replace inspection by a qualified technician.
      </p>
