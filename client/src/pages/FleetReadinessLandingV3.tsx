...
import {
  ReadinessPill,
  type Readiness,
} from "@/components/readiness/ReadinessPill";
    ...
    <div className="flex flex-col items-center">
      <div className="relative h-40 w-40 sm:h-44 sm:w-44">
        <svg
          ...
          role="img"
          aria-label={`Fleet health score ${score} percent`}
        >
          <circle
            cx="90"
            cy="90"
        ...
        <div className="absolute inset-0 flex flex-col items-center justify-center">
          ...
          <span className="mt-1 text-4xl font-black text-[#0A1A2E] sm:text-5xl">
            {score}%
          </span>
        </div>
      </div>
      <p className="mt-3 text-lg font-bold text-[#0A1A2E]">
        Fleet Health Score
      </p>
    </div>
  );
    ...
    <div className={cn(cardClass, "p-5 sm:p-6")}>
      <div className="mb-4 flex items-center justify-between">
        <p
          className={cn(
            monoClass,
            "text-[11px] font-bold uppercase tracking-wider text-[#73777E]"
          )}
        >
          Fleet Health
        </p>
        <span
          className={cn(
            monoClass,
            "rounded-full bg-[#F1F4F9] px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-[#73777E]"
          )}
        >
          Sample
        </span>
            ...
            return (
              ...
              >
                ...
                <div>
                  ...
                  <div className="mt-1 text-xs font-semibold text-[#73777E]">
                    {s.label}
                  </div>
                </div>
              </div>
      ...
      <div className="mt-4 flex items-start gap-2 border-t border-[#EEF1F5] pt-3">
        <CalendarClock
          className="mt-0.5 h-4 w-4 shrink-0 text-[#2D7DE0]"
          aria-hidden="true"
        />
        <p className="text-xs leading-5 text-[#38465F]">
          <span className="font-bold text-[#0A1A2E]">Follow-ups:</span> Unit 118
          defect awaiting sign-off · Unit 402 repeat fault
        </p>
      </div>
    </div>
  );
  ...
  return (
    <section className="bg-[#F6F8FB]">
      <div
        className={cn(
          sectionShell,
          "grid items-center gap-10 py-12 lg:grid-cols-[1.02fr_0.98fr] lg:py-20"
        )}
      >
        <div>
...
const LOSS_POINTS: Array<{ icon: LucideIcon; title: string; body: string }> = [
  {
    icon: Radio,
    title: "Reported, then lost",
    body: "A driver mentions a noise or a warning light — by phone, text, or paper — and it never reaches the person who can act on it.",
  },
  {
    icon: AlertTriangle,
    title: "Delayed until urgent",
    body: "Small issues wait behind everything else and only get attention once they become a roadside failure or an out-of-service truck.",
  },
  {
    icon: Compass,
    title: "Poorly prioritized",
    body: "Without a shared view of urgency, the loudest problem wins — not the one most likely to cause downtime.",
  },
  {
    icon: RefreshCw,
    title: "Left unresolved",
    body: "Repairs get closed without confirmation, the same fault returns, and repeat work quietly eats the maintenance budget.",
  },
];
          ...
          {LOSS_POINTS.map(p => {
            ...
            return (
              <div key={p.title} className={cn(cardClass, "p-5")}>
                ...
                <h3 className="mt-3 text-sm font-bold text-[#0A1A2E]">
                  {p.title}
                </h3>
                <p className="mt-1.5 text-sm leading-6 text-[#38465F]">
                  {p.body}
                </p>
              </div>
            );
...
const REVIEW_OUTCOMES: Array<{
  ...
  {
    icon: Search,
    title: "Your biggest workflow weakness",
    body: "A clear read on where issues are being lost, delayed, poorly prioritized, or left unresolved in your current process.",
  },
  {
    icon: ClipboardCheck,
    title: "One or two practical fixes",
    body: "Recommendations you can apply to your process right away — with or without TruckFixr.",
  },
  {
    icon: Gauge,
    title: "A demo tailored to your gaps",
    body: "We show only the parts of TruckFixr connected to the problems we uncover — not a generic platform tour.",
  },
];
      ...
      <div className={sectionShell}>
        ...
        <div className="mt-8 flex flex-col items-start gap-3">
          ...
          <p className="text-sm text-[#73777E]">
            ...
            paid pilot afterward. No obligation to proceed.
          </p>
        </div>
      </div>
    </section>
...
const FIT: Array<{ icon: LucideIcon; title: string; body: string }> = [
  {
    icon: Users,
    title: "Fleet owners & operations managers",
    body: "The people responsible for keeping commercial vehicles moving and maintenance decisions on track.",
  },
  {
    icon: Truck,
    title: "Commercial fleets, 5–50 vehicles",
    body: "Big enough that issues slip through the cracks, small enough that one missed repair hurts.",
  },
  {
    icon: Wrench,
    title: "Older diesel & mixed assets",
    body: "Especially operators running older diesel trucks and mixed commercial assets that need constant attention.",
  },
];
...
const STATES: Array<{ r: Readiness; body: string }> = [
  ...
  {
    r: "monitor",
    body: "May operate, but a known condition requires monitoring.",
  },
  {
    r: "service_soon",
    body: "Inspection, scheduling, or maintenance follow-up is required.",
  },
  {
    r: "stop",
    body: "Should not be dispatched until the concern is assessed or resolved.",
  },
];
const WORK_STEPS: Array<{ icon: LucideIcon; title: string; body: string }> = [
  {
    ...
    body: "Drivers report symptoms, warning lights, fault codes, or inspection findings — one place, not scattered calls and paper.",
  },
  {
    icon: Gauge,
    title: "Prioritize what matters",
    body: "Each vehicle gets a readiness state so the right issue rises to the top before dispatch.",
  },
  {
    icon: Compass,
    title: "Assign & track the repair",
    body: "Turn an issue into a tracked action — assigned, followed, and visible until it's done.",
  },
  {
    icon: ClipboardCheck,
    title: "Confirm the outcome",
    body: "Close the loop with a confirmed resolution, so repeat faults and unresolved work stop hiding.",
  },
];
...
function HowItWorksSection() {
  return (
    <section
      id="how-it-works"
      className="scroll-mt-16 bg-[#F6F8FB] py-16 sm:py-20"
    >
      <div className={sectionShell}>
        <SectionHeading
        ...
        <div className="mt-6 grid gap-6 lg:grid-cols-[0.9fr_1.1fr]">
          <div className={cn(cardClass, "p-5")}>
            <p className={cn(eyebrowClass, "!text-[#73777E]")}>
              Every vehicle, one clear status
            </p>
            <div className="mt-3 grid gap-3">
              {STATES.map(s => (
                <div
                  key={s.r}
                  className="flex items-start gap-3 rounded-md border border-[#E2E6EC] p-3"
                  style={{
                    borderLeftColor: READINESS_HEX[s.r],
                    borderLeftWidth: 3,
                  }}
                >
          ...
          <div className="grid content-start gap-4">
            <div
              className={cn(cardClass, "border-t-[3px] border-t-[#2D7DE0] p-5")}
            >
              <div className="flex items-center gap-2 text-[#2D7DE0]">
                <CalendarClock className="h-5 w-5" aria-hidden="true" />
                <p className={cn(eyebrowClass, "!text-[#2D7DE0]")}>
                  Scheduled maintenance
                </p>
              </div>
              <h3 className={cn(displayClass, "mt-2 text-xl")}>
                Know what&apos;s due before it&apos;s overdue
              </h3>
              <p className="mt-2 text-sm leading-6 text-[#38465F]">
                Track service due by mileage, engine hours, or date — whichever
                comes first — and stay ahead of PM and DOT / annual-inspection
                obligations. Missing readings show as &ldquo;insufficient
                data,&rdquo; never a false overdue.
              </p>
            </div>
            <div
              className={cn(cardClass, "border-t-[3px] border-t-[#D81F2A] p-5")}
            >
              ...
              <h3 className={cn(displayClass, "mt-2 text-xl")}>
                Surface problems that repeat or escalate
              </h3>
              <p className="mt-2 text-sm leading-6 text-[#38465F]">
                TruckFixr flags patterns worth a closer look — repeat repairs,
                returning warnings, unresolved defects, and vehicles consuming
                disproportionate maintenance — so they get attention before the
                next breakdown.
              </p>
            </div>
          </div>
        ...
        <div className="mt-6 flex flex-col gap-3 rounded-[10px] border border-dashed border-[#C3C7CE] bg-white p-4 sm:flex-row sm:items-start">
          <Route
            className="mt-0.5 h-5 w-5 shrink-0 text-[#73777E]"
            aria-hidden="true"
          />
          <p className="text-sm leading-6 text-[#38465F]">
            <span className="font-bold text-[#0A1A2E]">
              Being tested and expanding:
            </span>{" "}
            telematics integrations, automated mileage / engine-hour updates,
            and deterioration trends. TruckFixr does not claim exact failure
            dates or guaranteed breakdown prevention, and does not replace
      ...
      <div className={sectionShell}>
        ...
        >
          <div>
            ...
            <h2 className={cn(displayClass, "mt-2 text-3xl sm:text-4xl")}>
              Built by someone who has lived the maintenance workflow.
            </h2>
            <p className="mt-4 text-base leading-7 text-[#38465F]">
              TruckFixr comes out of hands-on experience managing commercial
              fleets and heavy-duty repair operations — the calls, the paper,
              the repeat faults, and the repairs that close without ever being
              confirmed.
            </p>
            <p className="mt-3 text-base leading-7 text-[#38465F]">
              Your review is led by{" "}
              <span className="font-bold text-[#0A1A2E]">Dickson Kwesiga</span>,
              founder of TruckFixr Fleet AI. You&apos;re booking a working
              session with the founder — not a demo request routed to a sales
              rep.
            </p>
            <div className="mt-6">
          ...
          <div className={cn(cardClass, "p-6")}>
            <div className="flex items-center gap-4">
              ...
              <div>
                <p className="text-base font-bold text-[#0A1A2E]">
                  Dickson Kwesiga
                </p>
                <p className="text-sm text-[#73777E]">
            ...
            <ul className="mt-5 space-y-3">
              {[
                ...
                <li
                  key={t}
                  className="flex gap-2 text-sm leading-6 text-[#38465F]"
                >
                  <ShieldCheck
...
const FAQS: Array<{ q: string; a: string }> = [
  ...
  {
    q: "Is this just a sales demo?",
    a: "No. The goal is to find a real weakness in your maintenance workflow and give you something useful to take away. A tailored demonstration is part of it, but only against the problems we actually uncover.",
  },
  {
    q: "Is it really free? What's the catch?",
    a: "The review is free and there's no obligation. If there's a strong operational fit, we may recommend a small, standardized paid pilot afterward — but that's a decision for after the call, not a condition of it.",
  },
  {
    q: "Who should be on the call?",
    a: "The person who owns or influences maintenance decisions — a fleet owner, operations manager, or maintenance manager for a commercial fleet with roughly 5–50 vehicles.",
  },
  {
    q: "Does TruckFixr replace a technician?",
    a: "No. It provides decision support and does not replace inspection, diagnosis, or a qualified technician, and does not provide roadworthiness certification.",
  },
  {
    q: "Do I need telematics hardware to start?",
    a: "No. A driver report, inspection, or warning light is enough. Automated telematics ingestion is being tested and expanded.",
  },
];
          ...
          {FAQS.map(f => (
            <details key={f.q} className="group py-4">
              <summary className="flex cursor-pointer list-none items-center justify-between text-[15px] font-bold text-[#0A1A2E]">
                {f.q}
                <span className="ml-4 text-[#D81F2A] transition-transform group-open:rotate-45">
                  +
                </span>
              </summary>
    ...
    <section className="bg-[#0A1A2E] py-14 sm:py-16">
      <div className={cn(sectionShell, "text-center")}>
        <h2
          className={cn(
            displayClass,
            "mx-auto max-w-3xl text-2xl !text-white sm:text-3xl"
          )}
        >
          Find the costly weakness in your maintenance workflow—in 25 minutes
          with the founder.
        </h2>
        <p className="mx-auto mt-4 max-w-xl text-sm leading-7 text-[#abcaea]">
          Free 25-minute founder-led review for qualified commercial fleets with
          5–50 vehicles. No obligation.
        </p>
  ...
  return (
    <div className="fixed inset-x-0 bottom-0 z-40 border-t border-[#C3C7CE] bg-white/95 p-3 backdrop-blur sm:hidden">
      <FleetReviewButton
        ...
        className="h-12 min-h-0 w-full justify-center py-0"
      />
    </div>
  );
}
...
export default function FleetReadinessLandingV3() {
  useSeoMeta({
    title:
      "TruckFixr Fleet AI | Stop Maintenance Issues From Becoming Costly Downtime",
    description:
