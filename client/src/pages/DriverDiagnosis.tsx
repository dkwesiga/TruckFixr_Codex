...
import { AlertTriangle, ChevronLeft, Sparkles, Stethoscope, Wrench } from "lucide-react";
...
function DriverDiagnosisContent() {
  ...
  const fleetId = params.get("fleet") ?? "1";
      ...
      <div className="min-h-screen bg-slate-50">
        <main className="mx-auto max-w-2xl px-4 py-12 sm:px-6 lg:px-8">
          <Card className="border-red-200 bg-red-50">
            <CardHeader>
              <CardTitle>Diagnostics require a vehicle</CardTitle>
              <CardDescription>
                Start diagnosis from the driver dashboard or a specific vehicle card so TADIS can use the correct vehicle context.
              </CardDescription>
            </CardHeader>
            <CardContent>
              <Button onClick={() => (window.location.href = "/driver")}>Back to Dashboard</Button>
            </CardContent>
          </Card>
        </main>
      </div>
        ...
        <div className="space-y-6">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                {diagnosisStarted && diagnosis ? "Generated from the details above." : "No diagnosis generated yet."}
              </CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              {diagnosisStarted && diagnosis ? (
                <>
                  <div
                  ...
                  {diagnosis.next_action === "ask_question" ? (
                    <div className="rounded-lg border border-blue-200 bg-blue-50 p-4">
                      <p className="text-xs font-semibold uppercase tracking-wide text-blue-700">Clarifying question</p>
                      <p className="mt-2 text-sm font-medium text-slate-900">{diagnosis.clarifying_question}</p>
                      <div className="mt-4 space-y-3">
                        <Textarea
                          value={clarificationAnswer}
                          onChange={(event) => setClarificationAnswer(event.target.value)}
                          placeholder="Answer this one question so TADIS can reduce uncertainty."
                        ...
                        <Button
                          ...
                          onClick={() => {
                            const nextHistory = [
                              ...
                              { question: diagnosis.clarifying_question, answer: clarificationAnswer.trim() },
                            ];
                  ...
                  <div>
                    <p className="text-xs font-semibold uppercase tracking-wide text-slate-500">Structured JSON</p>
