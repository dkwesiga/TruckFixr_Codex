...
> **⚠️ Status: inactive / historical.** The PostHog integration described below is
> **not currently installed** — `posthog-js` is not a dependency and
> `client/src/lib/analytics.ts` is a first-party no-op stub (it logs in dev and
...
This document describes the (historical) PostHog analytics integration for TruckFixr Fleet AI.
