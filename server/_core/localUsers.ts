...
export async function upsertLocalUser(user: InsertUser) {
  ...
  const nextUser: LocalUserRecord = {
    ...
    internalAdminRole: user.internalAdminRole ?? existing?.internalAdminRole ?? null,
    ownerOperatorMode: user.ownerOperatorMode ?? existing?.ownerOperatorMode ?? false,
    ...
    billingCadence: user.billingCadence ?? existing?.billingCadence ?? "monthly",
    ...
    stripePriceId: user.stripePriceId ?? existing?.stripePriceId ?? null,
    ...
    trialStart: user.trialStart ?? existing?.trialStart ?? null,
    trialEnd: user.trialEnd ?? existing?.trialEnd ?? null,
