...
export async function sendPasswordResetWithSupabaseEmail(input: {
  email: string;
  redirectTo: string;
}) {
  const payload = await requestSupabaseAuth("/auth/v1/recover", {
    method: "POST",
    body: JSON.stringify({
      email: input.email.trim().toLowerCase(),
...
export async function updateSupabasePasswordWithAccessToken(input: {
  accessToken: string;
  password: string;
}) {
  if (!hasSupabaseEmailAuth()) {
    return {
      success: false,
      error: "Supabase email auth is not configured",
    };
  }

  const response = await fetch(`${getAuthBaseUrl()}/auth/v1/user`, {
    method: "PUT",
    headers: {
      apikey: ENV.supabaseAnonKey,
