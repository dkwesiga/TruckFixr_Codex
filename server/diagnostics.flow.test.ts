...
function createIntakeInterpretation(overrides: Partial<Record<string, unknown>> = {}) {
  return {
    normalized_symptoms: ["engine overheating"],
    primary_symptoms: ["engine overheating"],
    secondary_symptoms: ["temperature rises on route"],
    ...
    likely_failure_modes: ["thermostat restriction", "coolant loss"],
    maintenance_history_signals: [],
    repair_history_signals: [],
    recent_parts_signals: [],
    recurrence_signals: [],
    evidence_keywords: ["engine overheating", "p0128", "cabin heat"],
    candidate_cause_hints: ["thermostat stuck closed or opening late", "coolant leak or low coolant level"],
    risk_flags: [],
    ...
    interpretation_rationale: ["Cooling symptoms need one clarifying answer before finalization."],
    ...overrides,
  };
}
...
describe("diagnostics router full loop", () => {
  beforeEach(() => {
    ...
    const responses = [
      ...
      createIntakeInterpretation({
        secondary_symptoms: ["temperature rises on route", "cab heat stays weak while overheating"],
        evidence_keywords: ["engine overheating", "p0128", "weak cab heat"],
        candidate_cause_hints: ["thermostat stuck closed or opening late"],
        missing_evidence: [],
        ambiguity_drivers: [],
        interpretation_rationale: ["Weak cab heat while overheating makes thermostat restriction the leading path."],
      }),
      {
    ...
    vi.stubGlobal(
      ...
      vi.fn(async (input: string | URL | Request) => {
        ...
        if (/openrouter\.ai|generativelanguage\.googleapis\.com/i.test(url)) {
          return createLlmResponse(responses[Math.min(responseIndex++, responses.length - 1)]);
        }
