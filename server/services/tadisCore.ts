...
export const DiagnosticVehicleSchema = z.object({
  ...
  isPoweredVehicle: z.boolean().optional(),
...
const SIMPLE_TADIS_DIAGNOSIS_MAX_TOKENS = 380;
...
function inferComplaintDomains(text: string, faultCodes: string[]) {
  ...
  // A FLASHING/blinking check-engine (MIL) or emissions light is the
  // industry-standard indicator of an active misfire severe enough to risk
  // catalytic converter damage — distinct from a steady/solid light, which
  // just means a fault code is stored. Route it to the fuel/misfire domain
  // so it doesn't fall through to an unrelated cause (e.g. cooling system)
  // by default when no other keyword matches.
  if (
    /(flash|flashing|blink|blinking)/.test(text) &&
    /(check engine|malfunction indicator|\bmil\b|emissions light|service engine|engine light)/.test(text)
  ) {
    domains.add("fuel");
  }
...
// Builds a ranked-cause entry directly from the AI's own diagnosis text when
// it doesn't match any CAUSE_LIBRARY entry (novel_cause_support_score /
// is_new_cause exist in the schema for exactly this — see enrichLlmRanking(),
// which already does this for the detailed/non-simple engine). The AI's
// actual reasoning is preserved as cause_name rather than silently swapped
// for the static rule-engine's top guess, which is prone to being wrong or
// irrelevant for symptoms the hardcoded keyword library has no entry for.
function buildNovelSimpleRankedCause(topLikelyCause: string, baselineStage: BaselineStage) {
  const evidence = baselineStage.evidence;
  return rankedCauseSchema.parse({
    cause_id: null,
    cause_name: topLikelyCause,
    is_new_cause: true,
    probability: 100,
    evidence_summary: [`AI diagnosis identified this directly from the reported symptoms.`],
    ranking_rationale: [
      "This cause was generated by the AI diagnosis model and did not match an entry in the static cause library.",
    ],
    symptom_support_score: Math.min(100, evidence.symptomScore),
    fault_code_support_score: Math.round(evidence.faultCodeScore * 0.5),
    repair_history_support_score: Math.min(100, evidence.repairHistoryScore),
    maintenance_history_support_score: Math.min(100, evidence.maintenanceHistoryScore),
    recent_parts_support_score: Math.min(100, evidence.recentPartsReplacedScore),
    recurring_failure_support_score: Math.min(100, evidence.recurringFailureScore),
    cause_library_fit_score: 0,
    novel_cause_support_score: 70,
    candidate_source_reasons: ["Introduced by the AI diagnosis layer"],
  });
}
...
function buildSimpleTadisQuestionOutput(input: DiagnosticInputRequest, baselineStage: BaselineStage, details: {
  ...
  // Deliberately does NOT fall back to baselineStage.ranked[0]'s CauseDefinition
  // when topCauseDef is null (an AI-generated novel cause) — that would leak
  // recommended tests/parts guidance for an unrelated rule-library cause.
  // getBaselineGuidance(null, causeName) already has a generic fallback that
  // stays grounded in the actual cause name.
