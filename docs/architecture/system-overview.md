...
| Parts lifecycle (requirement → identification → fitment → sourcing → supplier option → comparison → recommendation → human approval → order → receipt → install → outcome) | **Requirement through human approval now exists** (Parts Intelligence: `partIntelligence.ts` router, `parts`/`partRequirements`/`partFitmentAssessments`/`partSupplierOptions`/`partOptionApprovals` tables, deterministic fitment + comparison engines, owner/manager-gated approval, tenant-scoped, tested — `server/services/partsIntelligence.e2e.test.ts`). Order execution/receipt/install remain unbuilt by design (Phase 3+) — see `docs/architecture/parts-acquisition.md`. The older `partsRequests.ts` staff concierge flow is separate and unchanged. |
